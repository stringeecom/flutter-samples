...
import 'package:ios_call_notification_sample/managers/client_manager.dart';
...
class CallkeepManager {
  ...
  Future<void> reportIncomingCallIfNeeded(bool isCall1, StringeeCall? call1, StringeeCall2? call2) async {
    if (isCall1) {
      CallInfo callInfo = await callkeep.getCallInfo(call1?.id ?? '', call1?.serial ?? 1);
      debugPrint('call info ${callInfo.state}');
      if (callInfo.uuid != null && callInfo.uuid!.isNotEmpty) {
        if (callInfo.state == null) {
          await callkeep.displayIncomingCall(callInfo.uuid ?? '' , 'Stringee', hasVideo: call1?.isVideoCall ?? false, localizedCallerName: call1?.fromAlias ?? 'Stringee User');
      ...
      CallInfo callInfo = await callkeep.getCallInfo(call2?.id ?? '', call2?.serial ?? 1);
      if (callInfo.uuid != null && callInfo.uuid!.isNotEmpty) {
        if (callInfo.state == null) {
          await callkeep.displayIncomingCall(callInfo.uuid ?? '' , 'Stringee', hasVideo: call2?.isVideoCall ?? false, localizedCallerName: call2?.fromAlias ?? 'Stringee User');
        } else if (callInfo.state == CallState.answered) {
          CallManager().answer();
        } else if (callInfo.state == CallState.ended) {
          CallManager().endCall(false);
  ...
  Future<void> answerCallKeepIfNeed(bool isCall1, StringeeCall? call1, StringeeCall2? call2) async {
    CallInfo? callInfo;
    if (isCall1) {
      callInfo = await callkeep.getCallInfo(call1?.id ?? '', call1?.serial ?? 1);
    } else {
      callInfo = await callkeep.getCallInfo(call2?.id ?? '', call2?.serial ?? 1);
    }
  ...
  Future<void> endCallKeepIfNeed(bool isCall1, StringeeCall? call1, StringeeCall2? call2) async {
    CallInfo? callInfo;
    if (isCall1) {
      callInfo = await callkeep.getCallInfo(call1?.id ?? '', call1?.serial ?? 1);
    } else {
  ...
  void configureCallKeep()  {
    ...
    callkeep.on(CallKeepPushKitToken(), (event) {
      ...
      ClientManager().registerCallPush(event.token ?? '');
    });
    ...
    callkeep.on(CallKeepPerformAnswerCallAction(), (event) {
      (String, int)? currentCall = CallManager().currentCallIdAndSerial();
      callkeep.getCallInfo(currentCall?.$1 ?? '', currentCall?.$2 ?? 1).then((value) =>
      {
    ...
    callkeep.on(CallKeepPerformEndCallAction(), (event) {
      ...
      {
        if (event.callUUID == value.uuid) {
          CallManager().endCall(value.state == CallState.answered)
        }
      });
    });
