...
class IOSCallManager with WidgetsBindingObserver {
  ...
  SyncCall? syncCall;
  List<String?> _fakeCallUuids =
      []; // mảng các uuid của các call được show từ callkit mà cần end ngay sau khi show thành công (xử lý cho rule mới trên iOS 13)
  List<SyncCall?> _oldSyncCalls =
      []; // mảng các syncCall đã xử lý rồi, sẽ không xử lý lại nữa (xử lý cho StringeeX)
  ...
  GlobalKey<CallScreenState>?
      callScreenKey; // Tham chiếu đến CallScreen để có thể cập nhật giao diện khi cần
  AppLifecycleState appLifecycleState = AppLifecycleState.resumed;
  BuildContext? contextToShowCallScreen;
  ...
  void addListenerForCall() {
    ...
    if (syncCall!.stringeeCall != null &&
        !syncCall!.stringeeCall!.eventStreamController.hasListener) {
    ...
    if (syncCall!.stringeeCall2 != null &&
        !syncCall!.stringeeCall2!.eventStreamController.hasListener) {
  ...
  void showFakeCall() {
    ...
    **/
    ...
    callKeep.displayIncomingCall(fakeCallUuid, "Stringee",
  ...
  Future<void> didReceivePushNotification(
      CallKeepPushKitReceivedNotification event) async {
    print(
